...
def normalize_gesture_3d(gesture, axis_length=None):
    x, y, z = zip(*gesture)
...
def select_landmarks_3d(landmark_history: dict[int, list[tuple[int, int, int]]]):
    ...
    thresh = 0.08
    for landmark_id, tracking_points in landmark_history.items():
        normalized = normalize_gesture_3d(tracking_points, axis_length=max_axis_length)
        ...
        if x_var > thresh or y_var > thresh or z_var > 0.02:
...
def process_landmarks_3d(landmark_history: dict[..., list[tuple[int, int, int]]], include_landmarks: set[int] = None,
    ...
    good_landmarks = select_landmarks_3d(landmark_history)
    ...
    for landmark_id in good_landmarks:
        ...
        smoothed_points = np.array(gaussian_filter_3d(simplified, sigma=2))
